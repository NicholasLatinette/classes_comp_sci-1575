...
void fill_matrix(int rows, int cols, char **matrix)
{
    ...
    {
        char o;
        cout << "What character for row: " << i << " col 0?" << endl;
        cin >> o;
        matrix[i][0] = o;
        for(int x = 1; x<cols; x++)
        {
            cout << "What char for row: " << i << " col " << x << endl;
            char p;
            cin >> p; 
            matrix[i][x] = p;
        }
...
/* OKAY New Plan. Read in each row and column into a string and then
test the string for the word.
*/

void matrix_search(int sol[], string word, int rows, int cols, char **matrix)
{
    // Define your function here
    //have a loop that goes through each letter with one if statement, like this
    int i = 0;
        ...
        for(int u = 0; u < rows; u++)
        {
            if(word[p] == matrix[i][u])
            {
                ...
                for(n = 1; n <= amount_of_char; n++)
                {
                    ...
                    if(d+1 < rows && matrix[d+1][o] == word[n]) //*** This is a left to right search
                    {
                        cout << "yo1" << endl;
                        d++;
                        if(word[n+1] == '\0')
                    ...
                    //cout << "cp1" << endl;
                    if((d-1 > -1) && matrix[d-1][o] == word[n]) //*** This is a right to left search
                    {
                        d--;
                        if(word[n+1] == '\0')
                        {
                    ...
                    if(o+1 < cols && matrix[d][o+1] == word[n]) //*** This is a bot to top search
                    {
                        cout << "logic works" << endl;
                        o++;
                        if(word[n+1] == '\0')
                        {
                            word_found = true;
                    ...
                    if((o-1 > -1) && matrix[d][o-1] == word[n]) //*** This is a top to bot search
                    {
                        d--;
                        if(word[n+1] == '\0')
                        {
    ...
    while (word[i] != '\0') 
    {
        amt_of_char++;
        i++;
        //Try to create a way to go through each individual letter of the 
        //string, then create a bunch of if statements that go through each
        //searching algorithm
    } // This loop gets amount of chars, now need to create for loop that 
    //will look through each of ways that the program needs to search (forward, backward, oplique left right up down, up, down)
    for (int i = 0; i < rows; i++)
    {
        for(int x = 0; x < cols; x++)
        {
            ...
            if (matrix[i][x] == word[p])
            {
              ...
              //  matrix[w][e];
                w++;
                e++;
                if(matrix[w][e] == word[p] && w > rows && e > cols )
                {
                ...
                //then... all the searching algorithms
            }
        }
    }
